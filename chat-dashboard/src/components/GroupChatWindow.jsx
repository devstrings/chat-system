...
useEffect(() => {
  ...
  const handleReceiveGroupMessage = (msg) => {
    ...
    // SIMPLE check - only 2 conditions needed
    const matchesGroup = 
      msg.groupId === group._id || 
      msg.conversationId === group._id;
    ...
    if (matchesGroup) {
      ...
      console.log(" Message for different group, ignored");
    }
  };
  ...
  const handleGroupMessageDeletedForEveryone = ({ messageId, groupId }) => {
    console.log(" Group message deleted for everyone:", messageId);
    if (groupId === group._id) {
      setMessages(prev => prev.filter(m => m._id !== messageId));
    }
  };
  ...
  socket.on("groupMessageDeletedForEveryone", handleGroupMessageDeletedForEveryone);
  ...
  return () => {
    ...
    socket.off("groupMessageDeletedForEveryone", handleGroupMessageDeletedForEveryone);
  };
